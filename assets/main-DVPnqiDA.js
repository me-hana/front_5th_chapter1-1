...
`},h={"#/":d,"#/login":c,"#/profile":g},u=()=>{if(!location.hash||location.hash==="#"){location.hash="#/";return}const e=location.hash||"#/",s=h[e],l=JSON.parse(localStorage.getItem("user"));if(s)if(location.hash==="#/login"&&l){document.querySelector("#root").innerHTML=d(),location.hash="#/";return}else if(location.hash==="#/profile"&&!l){document.querySelector("#root").innerHTML=c(),location.hash="#/";return}else{document.querySelector("#root").innerHTML=s();return}else{document.querySelector("#root").innerHTML=f();return}};window.addEventListener("hashchange",u);window.addEventListener("DOMContentLoaded",u);const v=e=>{if(e.target.id==="logout")console.log("여기로 1"),e.preventDefault(),location.hash="#/login",localStorage.removeItem("userInfo");else{console.log("여기로 2");const s=e.target.closest("a[data-link]");if(s){console.log("여기로 3"),e.preventDefault();const r=s.getAttribute("href").split("#")[1];location.hash=r?`#${r}`:"#/"}}},x=e=>{var s,l,r,t;if(e.preventDefault(),e.target.id==="profile-form"){const o=((s=document.querySelector("#username"))==null?void 0:s.value.trim())||"",a=((l=document.querySelector("#email"))==null?void 0:l.value.trim())||"",m=((r=document.querySelector("#bio"))==null?void 0:r.value.trim())||"",b={username:o,email:a,bio:m};localStorage.setItem("user",JSON.stringify(b))}else if(e.target.id==="login-form"){const a={username:((t=document.querySelector("#username"))==null?void 0:t.value.trim())||"",email:"",bio:""};localStorage.setItem("user",JSON.stringify(a)),location.hash="#/"}};document.addEventListener("click",v);document.addEventListener("submit",x);
